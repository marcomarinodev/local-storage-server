...
int icl_hash_dump(FILE *stream, icl_hash_t *ht)
{
    ...
    printf("\n--- SERVER STATUS ---\n");
    printf("Current capacity: %ld bytes\nMax capacity %ld bytes\n", ht->current_size, ht->max_size);
    printf("\nFILES IN STORAGE\n");
    ...
    for (i = 0; i < ht->nbuckets; i++)
    {
        ...
        for (curr = bucket; curr != NULL;)
        {
            if (curr->key)
            {
                RR *data = (RR *) (curr->data);
                fprintf(stream, "\n*** icl_hash_dump: %s:\n\tCONTENT :\n%s\n\tIS OPEN : %d\n\tLST.CLNT:%d\n\tLST.EDIT: ", (char *)curr->key, data->content, data->is_open, data->last_client);
                fprintf(stream, "%d-%02d-%02d %02d:%02d:%02d\n", data->last_edit.tm_year + 1900, data->last_edit.tm_mon + 1, data->last_edit.tm_mday, data->last_edit.tm_hour, data->last_edit.tm_min, data->last_edit.tm_sec);
            }
            curr = curr->next;
        }
    }
